...
if ( ! class_exists( 'CAWP_Tools' ) ) {
	...
	class CAWP_Tools {
		...
		public static function set_cache( $name, $value, $expiration = 0) {
			...
			update_option( 'cawp_cache_' . $name, $option, 'no' );
		}
		...
		public static function delete_cache( $name ) {
			delete_option( 'cawp_cache_' . $name );
		}
		...
		public static function get_cache( $name ) {
			$option = get_option( 'cawp_cache_' . $name );
		...
		public static function clear_cache() {
			global $wpdb;
			$sqlquery = $wpdb->query( "DELETE FROM $wpdb->options WHERE option_name LIKE 'cawp_cache_qr%%'" );
		}
